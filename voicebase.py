...
class WhisperTinyVoice:
    def __init__(self):
        ...
        self.setup_whisper_base()
        
        print("=" * 70)
        print("  VOICE TO CLAUDE - WHISPER BASE + VAD")
        print("=" * 70)
        print("🔴 DESACTIVADO - El script ignora todas las teclas")
        print("")
        print("🎯 MODELO: Whisper Base (74MB)")
        print("⚡ VELOCIDAD: ~3-4s (rápido)")
        print("🎯 PRECISIÓN: ~92% (alta calidad + puntuación)")
    ...
    def setup_whisper_base(self):
        """Configura Whisper Base para balance velocidad/precisión"""
        try:
            print("🔄 Cargando Whisper Base...")
            
            # Configuración optimizada para balance
            self.model = WhisperModel(
                "base",                     # 🎯 Modelo base - mejor balance
            ...
            print("✅ Whisper Base cargado correctamente")
            print("🚀 Optimizado para velocidad máxima")
            print("🎯 Incluye puntuación automática")
            
        except Exception as e:
            print(f"❌ Error cargando Whisper Tiny: {e}")
            print("💡 Intenta: pip install faster-whisper")
            sys.exit(1)
    
    def transcribe_audio_tiny(self, audio_file):
        """Transcribe con Whisper Tiny ultra-optimizado"""
        try:
            start_time = time.time()
            print(f"🔄 Transcribiendo con Whisper Base...")
            ...
            print(f"⚡ Transcrito en {elapsed:.2f}s")
    ...
    def show_status(self):
        ...
        if not self.activated:
            ...
            status = "🟢 ACTIVADO - 🗣️ MODO SPEAK (WHISPER TINY)"
            commands = "ENTER=copiar | 's'=salir speak | Ctrl+L=desactivar"
        else:
            status = "🟢 ACTIVADO - 🎤 MANUAL (WHISPER TINY)"
            commands = "SPACE=hablar | 's'=speak | 'q'=salir | Ctrl+L=desactivar"
        
        print(f"\n{status}")
        print(f"🎯 Modelo: Whisper Tiny (rápido + puntuación)")
    ...
    def process_audio_tiny(self, for_speak_mode=False):
        """Procesa audio con Whisper Tiny"""
        if not self.frames:
            print("❌ No hay audio")
            return
        ...
        print(f"⚡ Procesando {duration:.1f}s con Whisper Tiny...")
        ...
        # Transcribir
        text = self.transcribe_audio_tiny(audio_file)
    ...
    def start_manual_mode(self):
        ...
        if success:
            self.process_audio_tiny(for_speak_mode=False)
        else:
